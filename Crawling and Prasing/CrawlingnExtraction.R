...
#get URL
Art_input<-getURL(Art_URL[-400],followlocation=T)
#Parsing
test_html<-list()
for (i in 1:length(Art_input)){
  test_html[[i]]<-htmlParse(Art_input[i])
}
...
for (i in 1:length(test_html)){
  saveXML(test_html[[i]],file=paste0('/Users/jeremyhui/Google Drive/School/CS636 - Data Analytics in R/HTML/',DOInames[i],'.html'))
}
